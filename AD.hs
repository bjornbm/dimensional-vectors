...
import qualified Numeric.AD as NAD (diff, lift)
...
diff f (Dimensional x) = Dimensional $ NAD.diff (undim . f . Dimensional) x
  where
    undim (Dimensional a) = a


class Lift w where lift :: (Num a, Mode tag) => w a -> w (AD tag a)
instance Lift (Dimensional v d) where lift (Dimensional x) = Dimensional (NAD.lift x)
